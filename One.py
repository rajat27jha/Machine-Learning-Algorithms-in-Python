...
# just for the example to see how it works how simple it is to change algo that we are using
...
#     pickle.dump(clf, f)
# a file named linearregression is made and opened with write mode
# now dump that trained clf in f
# to open that file open with read mode and input whats written in that file in a variale
# at last load that variable in clf
pickle_in = open('linearregression.pickle', 'rb')
clf = pickle.load(pickle_in)
# pickle is a native python object
# what pickle do is it serialize any python object ie converts objects to a series of
# bytes which can then be stored, and also deserialize it.
# we use pickle so as to not again and again train our classifier instead once trained we save it
